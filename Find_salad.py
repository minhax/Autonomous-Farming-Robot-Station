...
def circle_contour(image, contour):
    ...
    cX = int(M["m10"] / M["m00"])
    cY = int(M["m01"] / M["m00"])
    cv2.putText(image_with_ellipse, "center", (cX - 20, cY - 20),
		cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
    return image_with_ellipse,cX,cY


    

def find_salad(image):
    ...
    # Clean up
    #we want to circle our salad so we'll circle it with an ellipse
    ...
    # Find biggest salad
    #get back list of segmented salads and an outline for the biggest one
    big_salad_contour, mask_salads = find_biggest_contour(mask_clean)

    # Overlay cleaned mask on image
    # overlay mask on image, salad now segmented
    overlay = overlay_mask(mask_clean, image)

    # Circle biggest salad
    #circle the biggest one
    circled,coordX,coordY = circle_contour(overlay, big_salad_contour)
    show(circled)
    
    #we're done, convert back to original color scheme
    bgr = cv2.cvtColor(circled, cv2.COLOR_RGB2BGR)
    
    return bgr,coordX,coordY

#read the image
image = cv2.imread('C:/Users/Nguyen Cao Kha/Dev/test_images/test/Lettuce.jpg')
#detect it
result,cX,cY = find_salad(image)
#write the new image
cv2.imwrite('C:/Users/Nguyen Cao Kha/Dev/test_images/test/Lettuce2.jpg', result)
print(cX,cY)